...
async function migrateData() {
  ...
  try {
    ...
    for (const loc of mockData.locationDatabase) {
    ...
    for (const admin of mockData.users) {
      ...
      if (error) console.error('Error migrating admin:', error)
    }

    // 3. Migrate Shoppers
    console.log('Migrating shoppers...')
    for (const shopper of mockData.shoppers) {
      const { error } = await supabase.from('shoppers').insert({
        id: shopper.id,
        name: shopper.name,
        email: shopper.email,
        ...
        level: shopper.level,
        is_active: shopper.isActive,
        last_active: shopper.lastActive,
        created_at: new Date().toISOString()
      })
      if (error) console.error('Error migrating shopper:', error)
    }

    // 4. Migrate Visits
    console.log('Migrating visits...')
    for (const visit of mockData.visits) {
      // Find matching location id
      const { data: locData } = await supabase
        .from('locations')
        .select('id')
        .eq('name', visit.locationName)
        .eq('brand', visit.brand)
        .single()
      
      const { error } = await supabase.from('visits').insert({
        id: visit.id,
        shopper_id: visit.shopperId,
        ...
        status: visit.status,
        score: visit.score,
        issues_count: visit.issuesCount,
        notes: visit.notes,
        created_at: new Date().toISOString(),
