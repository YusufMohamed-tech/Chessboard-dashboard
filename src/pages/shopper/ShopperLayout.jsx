...
function getShopperTabs(unreadNotificationsCount = 0) {
  return [
    ...
    { label: 'الإشعارات', to: '/shopper/notifications', icon: BellRing, badge: unreadNotificationsCount },
    { label: 'التقارير', to: '/shopper/reports', icon: FileBarChart2 },
  ]
}

function getTitle(pathname) {
  if (pathname === '/shopper/dashboard') return 'لوحة تحكم الوكيل الميداني'
  if (pathname.includes('/shopper/visits/')) return 'تفاصيل الزيارة'
  if (pathname.includes('/shopper/completed/')) return 'تفاصيل الزيارة المكتملة'
  ...
  return 'لوحة تحكم الوكيل الميداني'
}
...
export default function ShopperLayout(props) {
  ...
  const linkedShopperIds =
    props.user?.linkedShopperIds?.length > 0 ? props.user.linkedShopperIds : [props.user?.id]

  const myVisits = props.visits.filter((v) => linkedShopperIds.includes(v.assignedShopperId))
  ...
  return (
    <div className="min-h-screen bg-cb-gray-50 py-4 md:py-6">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <section className="mt-4 rounded-2xl border border-cb-gray-200 bg-white p-3 shadow-sm">
              ...
              return (
                <NavLink key={tab.to} to={tab.to}
                  className={`flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-sm font-bold transition ${
                    isActive ? 'bg-cb-lime text-white' : 'bg-cb-gray-50 text-cb-gray-600 hover:bg-cb-gray-100'
                  }`}>
                  <Icon className="h-4 w-4" />
                  {tab.label}
                  {Number(tab.badge ?? 0) > 0 && (
                    <span className="inline-flex min-w-6 justify-center rounded-full bg-rose-100 px-1.5 py-0.5 text-xs font-black text-rose-700">{tab.badge}</span>
                  )}
                </NavLink>
              )
            })}
